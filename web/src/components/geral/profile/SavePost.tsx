...
export function SavePost() {
  ...
  const fetchData = async () => {
    try {
      const response = await api.get(`/artigoSave/${by}`)
          ...
          <main
            ...
            <footer className='h-[7vh] mx-10 flex pb-6 items-center justify-between'>
              <div className='lg:w-[24%] md:w-[22%] w-[50%] grid grid-cols-3 gap-5'>
                ...
                <span title='Avaliação Negativa' className='cursor-pointer'>
                  <Image
                    src={downIcon}
                    alt='Avaliação Negativa'
                    className='w-[26px] h-[26px] lg:w-[25px] lg:h-[25px] md:w-8 md:h-8 transition transform hover:translate-y-1 motion-reduce:transition-none motion-reduce:hover:transform-none '
                  />
                </span>
                <span title='Avaliação Positiva' className='cursor-pointer'>
                  <Image
                    src={upIcon}
